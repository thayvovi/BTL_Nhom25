...
class User{
	...
	static function checkUser($sdt){
		
			$db = DB::getInstance();

			$query = $db->prepare('SELECT * FROM users WHERE sdt = :sdt');
			$query->execute(array("sdt"=>$sdt));
			//$item=$query->fetch();
			if($query->rowCount() > 0)
			{
				$item = $query->fetch();
	...
	static function insert($ten_khach,$mat_khau,$sdt,$dia_chi,$level){
		try {
	    	//$gioi_tinh = 0;
			$db= DB::getInstance();
			$query = $db->prepare('INSERT INTO users SET ten_khach =:ten_khach, mat_khau=:mat_khau, sdt=:sdt, dia_chi=:dia_chi, level=:level');
			$query->execute(array("ten_khach"=>$ten_khach,"mat_khau" => $mat_khau,"sdt"=>$sdt,"dia_chi"=>$dia_chi,"level"=>$level));
	...
	static function update($id,$ten_khach,$mat_khau,$sdt,$dia_chi)
	{
		try {			
			$db = DB::getInstance();
			...
			$query->execute(array("ten_khach"=>$ten_khach,"mat_khau"=>$mat_khau,"sdt"=>$sdt,"dia_chi"=>$dia_chi,"id"=>$id));
